...
namespace Common.Kits
{
    public  class DrawImage2D
    {
        ...
        public Bitmap CreateImage()
        {
            ...
            if (intValuesCount % intKeysCount == 0)
            {
                ...
                for (int i = 0; i < intCurvesCount; i++)
                {
                    ...
                    CreateTitle(ref objGraphics,i,1);
        ...
        /// </summary>
        /// <param name="objGraphics"></param>
        public void CreateTitle(ref Graphics objGraphics,int i,int command)
        {
            objGraphics.DrawString(Title[i], new Font("宋体", 10), new SolidBrush(CurveColors[i]), new Point((int)(XSpace + 50), (int)(YSpace - YSlice / 2 - intFontSpace) + (i + 1) * 20));
            if (command > 0)
            {

                objGraphics.DrawLine(new Pen(CurveColors[i], 1), new Point((int)(XSpace + 50), (int)(YSpace - YSlice / 2 - intFontSpace) + (i + 1) * 20), new Point((int)(XSpace + 100), (int)(YSpace - YSlice / 2 - intFontSpace) + (i + 1) * 20));
            }
            else
            {
                objGraphics.DrawLine(new Pen(Color.White, 3), 65, 25 + 30, 100, 25 + 30);
