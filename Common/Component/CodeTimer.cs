namespace Common
{
    ...
    public static class CodeTimer
    {
        ...
        public static void Time(string name, int iteration, ActionDelegate action)
        {
            ...
            HttpContext.Current.Response.Write("\tCPU time:\t\t" + (ticks * 100).ToString("N0")
                              + "ns");
            HttpContext.Current.Response.Write("\tCPU time (one time):\t" + (ticks * 100 /
            ...
            for (int i = 0; i <= GC.MaxGeneration; i++)
            {
                ...
                HttpContext.Current.Response.Write("\tGen " + i + ": \t\t\t" + count);
            }
        ...
        public static void Time(string name, int iteration, IAction action)
        {
            ...
            // 4. Print CPU
            // Console.ForegroundColor = currentForeColor;
            HttpContext.Current.Response.Write("\tTime Elapsed:\t\t" + watch.ElapsedMilliseconds.ToString("N0") + "ms");
            HttpContext.Current.Response.Write("\tTime Elapsed (one time):" + (watch.ElapsedMilliseconds / iteration).ToString("N0") + "ms");


            HttpContext.Current.Response.Write("\tCPU time:\t\t" + (ticks * 100).ToString("N0") + "ns");
